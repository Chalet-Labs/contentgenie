...
- `getDashboardStats` per-counter failure isolation for the new unread-notifications counter is now handled with a `.catch(() => 0)` on the `countUnreadNotifications` call inside the same `Promise.all`. A wider `Promise.allSettled` migration for the other two counters is still potentially worthwhile but pre-existing and out of scope here.
...
- **Bell now dispatches** (`src/components/notifications/notification-bell.tsx`): `NOTIFICATIONS_CHANGED_EVENT` on `markAllNotificationsRead` success, when the cached count is `null`, `> 0`, **or** the just-fetched summary still reports unread rows (stale-zero recovery). Skips only when both signals agree there's nothing to mark. Prior to this PR the bell only listened.
