...
- Extracted the duplicated `(col AT TIME ZONE 'UTC') >= ... <= ...` window-filter helpers in `src/lib/observability/reconciliation-audit.ts` and `resolution-metrics.ts` into a shared `buildUtcWindowFilter(col, window)` at `src/lib/observability/window-filter.ts`. Replaced `rl_created_at_idx` on `reconciliation_log` with an expression index `((created_at AT TIME ZONE 'UTC') DESC)` (migration `drizzle/0031_broad_azazel.sql`) so the wrapped predicate stays index-eligible without losing TZ-independent semantics — addresses the codex audit-window perf finding on PR #446.
