...
### Fixed
- Android lock-screen / notification media controls now show `play + rewind + forward` instead of `play + skip-to-next`. Chrome on Android's compact notification slot ranks `nexttrack` above `seekforward`; since PR #348 introduced asymmetric −10s/+30s skip, the app now never registers `nexttrack`, so the seek buttons surface on the lock screen (Overcast / Pocket Casts behavior). Also exposes Media Session `seekto`, routed through the existing `api.seek()` clamp, so the lock-screen / CarPlay progress bar becomes draggable. Trade-off: advancing the queue from the lock screen now requires opening the app — accepted as deterministic vs. device/version-dependent heuristics (#355).
