...
- Extended observability dashboard at `/admin/topics/observability` with four new panels: **Match method trend** (stacked-bar chart of auto / llm_disambig / new counts per day or week), **Similarity trend** (20-bucket heatmap over time), **Resolution drift** (threshold-drift alert banner with ok / warn / alert status derived from configurable rate thresholds in `src/lib/observability/drift-thresholds.ts`), and **Reconciliation audit log** (per-run cluster table with outcome badges and Prev/Next pagination). New `granularity` and `auditPage` search params drive the time-series bucket width and audit pagination. Backed by four new query functions: `getMatchMethodTrend`, `getSimilarityTrend`, `detectThresholdDrift`, and `getReconciliationAuditLog` in `src/lib/observability/`. The nightly `reconcile-canonicals` task now generates a `runId` (`crypto.randomUUID()`), collects per-cluster audit rows via `ReconcileSummaryAccumulator`, and persists them to the new `reconciliation_log` table (migration `drizzle/0030_mute_shooting_star.sql`) via `insertReconciliationAuditRows`. Ships four new UI components (`MatchMethodTrendBars`, `SimilarityTrendHeatmap`, `DriftBanner`, `ReconciliationAuditTable`) each with Storybook stories and RTL unit tests. All time-window queries reinterpret the `timestamp without time zone` columns as UTC via `AT TIME ZONE 'UTC'` (and the 3-arg `date_trunc` form) so bucket alignment and window filtering remain correct in non-UTC sessions. (#392, ADR-053)
...
- Nightly `reconcile-canonicals` Trigger.dev scheduled task (cron `0 3 * * *` UTC) that catches duplicate canonical topics the resolver creates at ingestion time. Pipeline: DBSCAN-over-cosine clustering on identity embeddings (candidate generation) → LLM winner-pick + per-pair pairwise-verify (judge) → reuse of `mergeCanonicals` (each merge is its own transaction) → SQL decay flip for stale event-type kinds → structured `reconcile_summary` log. Adds a pinned `density-clustering@1.3.0` dependency and the `src/lib/reconcile-constants.ts` / `src/lib/reconcile-clustering.ts` modules + two new prompts (`reconcile-winner-pick`, `reconcile-pairwise-verify`) with Zod-validated outputs. See ADR-050 and #389.
- `backfill-canonical-topics` Trigger.dev task that re-extracts the canonical-topic layer from stored episode summaries (~100× cheaper than full re-summarization). Accepts `{ batchSize?, episodeIds?, dryRun? }`. Uses a three-layer idempotence guard (LEFT JOIN IS NULL selection + resolver ON CONFLICT DO NOTHING + `length(summary) >= 100` SQL floor). `dryRun=true` runs the LLM + normalizer but skips persistence — use for pre-flight distribution-drift checks before a full corpus run. Reuses `normalizeTopics` (A3) and `resolveAndPersistEpisodeTopics` (A5) with no duplicate resolver logic. Concurrency capped at 2 (lower than fresh-ingestion's 3); 500ms inter-episode delay. (#390, ADR-048)
- Lite admin UI for manually merging and unmerging canonical topics at `/admin/topics` (list) and `/admin/topics/[id]` (detail). Ships `mergeCanonicals` / `unmergeCanonicals` helpers in `src/trigger/helpers/database.ts` (reusable by the B1 reconciliation task), a `canonical_topic_admin_log` audit table (migration `drizzle/0027_open_silhouette.sql`), four new server actions (`adminMergeCanonicals`, `adminUnmergeCanonicals`, `getCanonicalTopicsList`, `getAdminAuditLog`) gated via a new `withAdminAction` wrapper, and a Topics tab in `AdminTabNav`. The merge transaction uses two sequential SQL statements (not a CTE) for the junction rewrite — see ADR-046 for rationale. (#385, ADR-046)
- Admin observability dashboard at `/admin/topics/observability`: match-method distribution, similarity histogram (0.05 buckets), and version-token-forced disambig count. Time-window selector: 24h / 7d / 30d (rolling). The junction `episode_canonical_topics` gains a `version_token_forced_disambig` boolean and an `updated_at` timestamp (migration `drizzle/0029_parallel_prodigy.sql`); `updated_at` advances on every `ON CONFLICT DO UPDATE`, and rolling-window queries filter on it so retries and recovery-path re-resolutions land in the window where they were observed. **Prod rollout requires running `scripts/backfill-junction-updated-at.sql` immediately after `bun run db:push` (with the resolver paused) so legacy rows reset `updated_at = created_at` — without it, all pre-migration rows count as freshly observed for up to 30 days.** (#387, ADR-047)
- Canonical-topic resolution now runs after `summarize-episode` summary persistence; new helper `resolveAndPersistEpisodeTopics` orchestrates batched embeddings and the per-episode 5-disambig budget. (#386, ADR-045)
- Clickable canonical-topic chips on episode cards linking to `/topic/[id]` stub pages. Each chip shows a kind-specific icon (Lucide: Megaphone, AlertOctagon, Scale, Handshake, Calendar, Lightbulb, Book, Tag), an outline Badge styled via `badgeVariants` applied directly to `<Link>` (no nested `<a>`), and `opacity-60` dimming for dormant topics. Chips appear on podcast detail, library, and any surface using the shared `EpisodeCard` primitive. The `/topic/[id]` page resolves merged-topic chains (cycle detection + `MAX_MERGE_DEPTH=16`) and redirects to the canonical URL via `permanentRedirect`. Top-3 chips per episode are sourced by a single window-function SQL query (`row_number() OVER (PARTITION BY episode_id ORDER BY coverage_score DESC)`) on the podcast-detail page, and via a Drizzle nested `with` query on the library page (#393).
...
- Entity-resolution module (`src/lib/entity-resolution.ts`) implementing the three-tier canonical-topic resolution pipeline (auto-match → LLM disambiguator → new insert) with `pg_advisory_xact_lock`-guarded transactions and a version-token pre-gate. Adds a `Pool`-backed Drizzle client at `src/db/pool.ts` (HTTP client unchanged) so the resolver can run a two-phase split — TX-1 (lock + exact-lookup + kNN) → LLM call outside any tx → TX-2 (re-acquire lock + finalize) — without holding the advisory lock across the OpenRouter round-trip. Includes a canonical `normalizeLabel` helper applied uniformly to lock-key, `normalized_label` writes, and exact-lookup; failure-mode discrimination so transport errors raise `EntityResolutionError("disambig_transport_failed")` and JSON parse / schema-validation failures raise `EntityResolutionError("disambig_parse_failed")` instead of silently minting canonicals; and the disambiguator prompt builder at `src/lib/prompts/entity-disambiguator.ts`. See ADR-044 and issue #384.
...
- `import "server-only"` markers across all server-key consumers in `src/lib/` (`embed.ts`, `assemblyai.ts`, `push.ts`, `rate-limit.ts`, `admin/stream-completion.ts`, `ai/providers/openrouter.ts`, `ai/providers/zai.ts`) so the Next.js RSC compiler fails the build if a `"use client"` component imports them — directly or transitively via `@/lib/ai`. Vitest aliases `server-only` to its empty stub so test environments still resolve cleanly. Trigger.dev's esbuild bundler gets the same alias via a `server-only-stub` `esbuildPlugin` extension in `trigger.config.ts`, since Trigger workers don't run through Next's RSC compiler and would otherwise crash at module load (the package's `index.js` is an unconditional `throw`). Dead `generateCompletion` re-export removed from `@/lib/openrouter` (file was already `@deprecated`); the unused re-export was the channel through which a future client import of the deprecated barrel would have leaked the server-only chain into the client bundle. `engines.node: ">=20"` pinned in `package.json` to formalize the runtime requirement that `AbortSignal.any` (used by `stream-completion.ts`) and the Vitest alias depend on.
- Test coverage for `src/app/actions/library.ts`, `src/app/actions/collections.ts`, and `src/components/audio-player/queue-panel.tsx`, lifting global line coverage above the enforced 80% threshold in `vitest.config.ts` so `bun run test:coverage` passes again. The new `library.ts` tests also pin the `where`-predicate user-id scope on destructive operations and the parsed-Zod payload that flows into the episode upsert (#372).
- Storybook test-runner smoke job in CI (#327). Every story is mounted in real headless Chromium via `@storybook/test-runner`; render-time errors, unhandled rejections, and `console.error` calls (via the `--failOnConsole` CLI flag) now fail the build, closing the gap that let PR #292's `setStorybookIsAdmin` dead-code regression slip through Vitest, `build-storybook`, and TypeScript. Adds a `bun run test:storybook` script for local use (assumes `bun run storybook` running on port 6006). Also adds `play()` assertions to `Layout/Sidebar > WithAdmin` and `> InSheetWithAdmin` that assert the admin link is visible — these stories now double as behavior tests alongside VRT.
...
- Extracted the duplicated `(col AT TIME ZONE 'UTC') >= ... <= ...` window-filter helpers in `src/lib/observability/reconciliation-audit.ts` and `resolution-metrics.ts` into a shared `buildUtcWindowFilter(col, window)` at `src/lib/observability/window-filter.ts`. Replaced `rl_created_at_idx` on `reconciliation_log` with a **concurrently-built** expression index `((created_at AT TIME ZONE 'UTC') DESC)` (migration `drizzle/0031_fat_thaddeus_ross.sql`) so the wrapped predicate stays index-eligible without losing TZ-independent semantics, and the deploy doesn't block the nightly reconciliation writer — addresses the codex audit-window perf finding and the copilot lock-on-deploy finding on PR #446.
- Extracted the data-normalization layer (`coerceEmbedding`) and three repository-style SQL helpers (`fetchActiveCanonicals`, `countEpisodesForCanonical`, `decayStaleCanonicals`) out of `src/trigger/helpers/reconcile-canonicals.ts` into focused sibling modules `src/trigger/helpers/coerce-embedding.ts` and `src/trigger/helpers/reconcile-canonicals-db.ts`. The orchestrator now reads as pure orchestration over named layers; new direct unit tests cover all driver-shape paths and SQL shapes. The `coerceEmbedding` string parser was tightened (trim-then-filter-empties) to fix a pre-existing inconsistency where `"[1,,2]"` parsed to `[1,2]` but `"[1, ,2]"` parsed to `[1,0,2]`; both now produce `[1,2]`. The DB-helper signatures use a new `DbExecutor = Pick<typeof RealDb, "execute">` type so test fakes don't need wide casts. Phase 7 decay SQL still uses the `kind = ANY(ARRAY[...]::canonical_topic_kind[])` form preserved from PR #433. (#435)
- Refactored `runReconciliation` into phase helpers (`pickWinner`, `verifyLosers`, `mergeVerifiedLosers`, `computeEpisodeDrift`) + `ReconcileSummaryAccumulator` for clarity. Removed the unreachable `resolveTransitiveLosers` / `winnerToAbsorbedLosers` pathway (its premise — direct DB re-pointing of an already-merged loser via `mergeCanonicals` — is rejected by the `not-active` guard, so the entire path was dead code; downstream chain resolution is handled at read time by `walkMergedChain`). No behavior change. (#434)
- Summarization output is now dual-layer: the existing broad-tag field is renamed `topics → categories` (shape unchanged: `{ name, relevance }[]`, still written to `episode_topics` for the existing UI ranking) and a new `topics: NormalizedTopic[]` field carries canonical-topic candidates with a typed `kind` taxonomy (`release | incident | regulation | announcement | deal | event | concept | work | other`). Both are produced in a single LLM call. The summarization prompt now also receives a top-50 category banlist (cached 1 h, module-scope) and instructs the model to reject any canonical-topic label that exact-matches a category. New helpers `src/lib/category-banlist.ts` and `src/lib/topic-label-validator.ts` filter out empty/oversized/control-char/instruction-shaped/banlisted labels before any DB write. The normalizer in `src/trigger/helpers/ai-summary.ts` is split into `normalizeCategories` and `normalizeTopics` and called under independent try/catch blocks so a parse failure on one layer cannot strand the other (mirrors ADR-031). No new DB writes; the new `topics` layer is consumed downstream by A4/A5. Foundation for #376. (#381)
- `pre-pr-validation` skill now loads `docs/code-review-checklist.md` as a supplementary rubric (Phase 2.0) and threads it into Codex review, `/pr-review-toolkit:review-pr all`, and `/simplify` with explicit framing — each reviewer is asked to cite `[checklist §N]` for rubric-derived findings while keeping its existing specialty. Missing-file is graceful: warn and continue, never fails the pipeline (#367).
- `docs/code-review-checklist.md` §5 strengthened with explicit "Reviewers MUST flag / MUST NOT defer" language and a Bad/Good example block that targets the rule-of-three follow-up-issue punt observed on PR #417. New §7 ("Reviewer scope: fix-now vs. defer-to-follow-up") makes the in-scope-by-default contract explicit: anything a PR introduces, modifies, or extends is in-scope for review and fix in that same PR; defer-to-follow-up is reserved for fixes that would touch files outside the PR's stated scope and are not required by the change. `AGENTS.md` Effort estimation cross-references both sections, and the three `pre-pr-validation` Phase 2.1 reviewer-launch directives (Codex, `/pr-review-toolkit:review-pr all`, `/simplify`) gain a one-sentence "MUST NOT defer §5 duplication to a follow-up issue" clause (#422).
- Auto-dismiss notifications (#362) review follow-up: `recordListenEvent` and the dismiss helper now return the episode ids actually flipped (`dismissedEpisodeDbIds: number[]`) — `ListenedButton` and `audio-player-context.onEnded` only fire `NOTIFICATIONS_CHANGED_EVENT` when the dismiss confirmed at least one row, so a silent dismiss-helper failure can no longer make the UI claim a removal that didn't happen on the server. The optimistic `addToQueue` dispatch and the empty-payload re-fetch fallback in `NotificationPageList` were removed; the debounced `setQueue` resolution is now the single dispatch site for queue-driven dismisses, eliminating the topic-losing re-fetch path. ADR-041 updated.
...
- Authenticated episode page restructured into a three-tab layout — **Insights** (Worth-It score, AI summary, key takeaways) · **Chapters** · **About** — with Insights as the default tab so the product's AI differentiator lands above the fold. The Chapters tab surfaces `chaptersUrl` data via `parseChapters` with click-to-seek into the audio player and active-chapter highlighting when playback is live; the tab is hidden when the episode has no `chaptersUrl`. New local primitive `src/components/episodes/episode-tabs.tsx` (underline style, count badges) keeps the existing shadcn segmented-pill `Tabs` untouched. See ADR-039.
...
- `drizzle-kit push` non-convergence (Phase 1): explicit names for three `canonical_topic_*` foreign keys whose auto-generated names exceeded Postgres's 63-character identifier limit and were silently truncated. Drizzle's diff didn't model the truncation, so every push re-emitted the same `DROP CONSTRAINT` + `ADD CONSTRAINT` for `canonical_topic_aliases`, `episode_canonical_topics`, and `canonical_topic_digests`. Migration `drizzle/0032_superb_bullseye.sql` renames in place via `ALTER TABLE ... RENAME CONSTRAINT` (no FK revalidation pass). Index drift for six expression/HNSW indexes is unrelated and tracked separately as Phase 2 (#451).
- Admin canonical-topics: the episode-count range filter now uses the qualified-outer-id helper (`canonicalTopicEpisodeCount()`) instead of an inline subquery whose bare `id` reference resolved to the inner junction PK, making the predicate effectively always-false; range URL params are also clamped at the page boundary so the action's Zod refine cannot be bypassed. The `triggerFullResummarize` server action now rejects synthetic/RSS feeds (non-numeric `podcastIndexId`) before flipping `summary_status → queued`, instead of triggering a NaN payload that stranded the row. The drift page surfaces backend errors as a destructive banner instead of silently rendering an empty "no drift" state. The `removeAlias` action returns `{ success: false, error: "not-found" }` when no row matches (preserving IDOR-defense) so the UI no longer shows a success toast on a no-op. Bulk-merge now logs unexpected per-loser errors (domain errors stay quiet) and rejects duplicate `loserIds` at the schema layer. Row selection in the topics table resets when the user paginates or changes filters. (#391)
- Public topic page, admin topics list, and merge dialog displayed `0 episodes` for canonical topics that had episodes. They now reflect the actual junction-row count (#419).
- `Podcasts/EpisodeList > Default` and `> SingleEpisode` Storybook stories now render correctly. After `EpisodeCard` adopted `PlayEpisodeButton` (#350), these stories crashed at render with `useAudioPlayerAPI must be used within AudioPlayerProvider` — but the failure was invisible because Vitest excludes `*.stories.tsx`, `build-storybook` only compiles, and the VRT baselines were captured against the broken state (proven by `Default` and `SingleEpisode` having byte-identical baselines despite rendering different episode counts). The newly added Storybook test-runner smoke job caught both. The fix wraps the meta in a minimal `AudioPlayerAPI/State/Progress` context decorator with noop API + idle state, mirroring the pattern already used by `audio-player/*.stories.tsx`. The two affected VRT baselines were regenerated by running the "Update VRT Baselines" workflow on this branch (#327).
- `NotificationPageList` no longer skips or duplicates a row when "Load more" is clicked while an optimistic dismiss is still in flight. The pagination offset is now decremented optimistically when the dismiss starts and rolled back if the server rejects it (#315).
