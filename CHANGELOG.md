...
- `pre-pr-validation` skill now loads `docs/code-review-checklist.md` as a supplementary rubric in a new Phase 2.0 step, then threads it into Codex review, `/pr-review-toolkit:review-pr all`, and `/simplify` with explicit framing (each reviewer is asked to cite `[checklist §N]` for any rubric-derived finding). The rubric is **additive** — every reviewer keeps its existing specialty; the rubric runs as a supplementary pass on top. Missing-file is graceful (warn + continue, never fails the pipeline) so the gate stays forward-compatible (#367).
