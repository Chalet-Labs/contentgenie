...
- Schema groundwork for `/subscriptions` sort options and pinning (epic #336, sub-issue #337): `user_subscriptions.is_pinned` boolean column (DEFAULT false NOT NULL) with a partial index `user_subscriptions_pinned_idx ON (user_id) WHERE is_pinned = true`; `users.preferences` typed JSON shape gains `subscriptionSort?: "recently-added" | "title-asc" | "latest-episode" | "recently-listened"`; new exported `SubscriptionSort` type alias in `src/db/schema.ts`. Migration `drizzle/0022_outgoing_agent_zero.sql`. Server-action and UI wiring follow in subsequent sub-issues.
