...
- `canonical_topics.episode_count` denormalized counter is removed; episode counts are now computed at read time via a correlated subquery against `episode_canonical_topics`. The counter was never bumped by the resolver path (`insertJunction()` in `src/lib/entity-resolution.ts`), so the public topic page, admin topics list, and merge dialog all displayed `0` for topics with junction rows. Drops the column and `ct_episode_count_gte_0` CHECK via migration `drizzle/0028_abandoned_bloodscream.sql`; replaces all four projections with the new `canonicalTopicEpisodeCount()` helper at `src/lib/admin/canonical-topic-episode-count.ts` (aliased to `episode_count` so `CanonicalTopicRow.episodeCount` stays stable). The merge/unmerge transaction in `src/trigger/helpers/database.ts` no longer writes the column. (#419)
