...
- `canonical_topics`, `canonical_topic_aliases`, and `episode_canonical_topics` schema with dual `vector(1024)` embeddings, HNSW indexes (`m=16, ef_construction=64`), partial unique index on `(lower(normalized_label), kind) WHERE status='active'`, six CHECK constraints on `canonical_topics` (biconditional merged flag, self-merge ban, relevance/episode_count ranges, non-blank label/summary), and three on the junction table (match_method enum, coverage_score/similarity ranges). Migration `drizzle/0024_elite_king_cobra.sql`. Foundation for canonical-topics entity resolution (#383).
