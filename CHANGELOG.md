...
- Test coverage for `src/app/actions/library.ts`, `src/app/actions/collections.ts`, and `src/components/audio-player/queue-panel.tsx`. All three files reach 100% line coverage; global line coverage rises from 78.86% to 81.82% so `bun run test:coverage` again passes the enforced 80% threshold in `vitest.config.ts` (#372).
