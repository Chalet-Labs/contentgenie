...
### Changed
- Optimized episode average rating retrieval by using SQL `avg()` and `count()` aggregates in a single query, and added a standalone index on `user_library.episode_id` for faster lookups.
