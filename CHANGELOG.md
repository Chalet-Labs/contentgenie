...
- Admin observability dashboard at `/admin/topics/observability`: match-method distribution, similarity histogram (0.05 buckets), and version-token-forced disambig count. Time-window selector: 24h / 7d / 30d (rolling). The junction `episode_canonical_topics` gains an `updated_at` column (migration `drizzle/0029_cynical_mimic.sql`) that advances on every `ON CONFLICT DO UPDATE`; rolling-window queries filter on `updated_at` so retries and recovery-path re-resolutions land in the window where they were observed. (#387, ADR-047)
