...
- Storybook test-runner smoke job in CI (#327). Every story is mounted in a real headless Chromium via `@storybook/test-runner`; render-time errors and unhandled rejections now fail the build, closing the gap that let PR #292's `setStorybookIsAdmin` dead-code regression slip through Vitest, `build-storybook`, and TypeScript. Adds a `bun run test:storybook` script for local use (assumes `bun run storybook` running on port 6006). Also adds `play()` assertions to `Layout/Sidebar > WithAdmin` and `> InSheetWithAdmin` that assert the admin link is visible — these stories now double as behavior tests alongside VRT. The CI `quality` job's prior single "Serve Storybook and run VRT" step is split into Serve / Smoke / VRT / Stop so each phase has its own status line.
