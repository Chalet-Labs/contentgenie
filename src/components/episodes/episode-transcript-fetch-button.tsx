...
export function EpisodeTranscriptFetchButton({
  ...
  const [isFetching, setIsFetching] = useState(
    initialTranscriptStatus === "fetching",
  );
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      const returnedDbId: number | null =
        typeof responseData?.episodeDbId === "number"
          ? responseData.episodeDbId
          : null;
      ...
      if (returnedDbId !== null) {
        ...
        setIsFetching(false);
        toast.error(
          "Transcript fetch started but status polling is unavailable — refresh the page to check progress",
        );
      }
    } catch (err) {
      setIsFetching(false);
