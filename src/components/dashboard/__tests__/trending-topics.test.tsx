...
describe("TrendingTopics", () => {
  ...
  it("renders topic description with line-clamp-2 class", () => {
    const topics = [
      makeTopic({ name: "AI", description: "AI is changing everything." }),
    ];
    render(<TrendingTopics topics={topics} generatedAt={fixedDate} />);
  ...
  it("omits description paragraph when description is empty string", () => {
    ...
    const { container } = render(
      <TrendingTopics topics={topics} generatedAt={fixedDate} />,
    );
    expect(screen.getByText("AI")).toBeInTheDocument();
  ...
  it("topic name element has no truncate class", () => {
    const longName =
      "This Is A Very Long Topic Name That Would Overflow In A Pill";
  ...
  it("displays card title 'Trending Topics' and subline 'Past 7 days · Updated 10m ago'", () => {
    ...
    expect(
      screen.getByText(/Past 7 days · Updated 10m ago/),
    ).toBeInTheDocument();
  });
  ...
  it("renders 'Out of date' suffix and warning color when isStale is true", () => {
    ...
    const desc = screen.getByText(
      /Past 7 days · Updated 10m ago · Out of date/,
    );
    expect(desc).toBeInTheDocument();
...
describe("TrendingTopicsLoading", () => {
  it("renders TOPICS_INITIAL row-shaped skeleton placeholders", () => {
    ...
    expect(screen.getAllByTestId("trending-loading-row")).toHaveLength(
      TOPICS_INITIAL,
    );
  });
});
