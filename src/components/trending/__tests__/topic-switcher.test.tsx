...
describe("TopicSwitcher", () => {
  it("renders one pill per topic", () => {
    render(
      <TopicSwitcher topics={topics} activeSlug="artificial-intelligence" />,
    );
  ...
  it("legacy topic missing slug uses slugify(name) for href", () => {
    ...
    render(
      <TopicSwitcher topics={[legacyTopic]} activeSlug="space-exploration" />,
    );
  ...
  it("empty topics array renders nothing", () => {
    const { container } = render(
      <TopicSwitcher topics={[]} activeSlug="anything" />,
    );
