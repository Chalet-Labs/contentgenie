...
vi.mock("@/app/actions/ai-config", () => ({
  updateSummarizationPrompt: (...args: unknown[]) =>
    mockUpdateSummarizationPrompt(...args),
}));

vi.mock("@/app/actions/admin", () => ({
  searchEpisodesWithTranscript: (...args: unknown[]) =>
    mockSearchEpisodes(...args),
}));
...
describe("PromptTemplateCard", () => {
  ...
  it("calls updateSummarizationPrompt with current textarea value on Save", async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateSummarizationPrompt).toHaveBeenCalledWith(
        "Analyze {{transcript}}",
      );
    });
  });
