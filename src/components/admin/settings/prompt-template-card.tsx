...
export function PromptTemplateCard({ initialPrompt }: PromptTemplateCardProps) {
  ...
  const [selectedEpisode, setSelectedEpisode] =
    useState<EpisodeSearchResult | null>(null);
  ...
  const missingTranscript =
    promptText.length > 0 && !promptText.includes("{{transcript}}");
  ...
  const handleSave = async () => {
    ...
    try {
      const normalizedPrompt =
        promptText.trim().length === 0 ? null : promptText;
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Customize the prompt used when generating episode summaries. Leave
          empty to use the built-in default. Test your prompt live against a
          real episode before saving.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {(testOutput || isTesting) && (
          <pre className="max-h-96 overflow-y-auto whitespace-pre-wrap rounded border bg-muted p-3 text-sm">
            {testOutput}
          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This will delete your custom prompt and revert to the built-in
                  default summarization prompt. This action cannot be undone.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
