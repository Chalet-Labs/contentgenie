...
export function EpisodeActionButtons({ episode }: EpisodeActionButtonsProps) {
  ...
  const [transcriptAccessToken, setTranscriptAccessToken] = useState<
    string | null
  >(null);
  const [summaryRunId, setSummaryRunId] = useState<string | null>(null);
  const [summaryAccessToken, setSummaryAccessToken] = useState<string | null>(
    null,
  );

  const isCombinedRef = useRef(false);
  const handleSummarizeRef = useRef<() => Promise<void>>(() =>
    Promise.resolve(),
  );

  const { run: transcriptRun, error: transcriptError } = useRealtimeRun<
    typeof fetchTranscriptTask
  >(transcriptRunId ?? "", {
    accessToken: transcriptAccessToken ?? "",
    enabled: !!transcriptRunId && !!transcriptAccessToken,
  });

  const { run: summaryRun, error: summaryError } = useRealtimeRun<
    typeof summarizeEpisode
  >(summaryRunId ?? "", {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (
          transcriptInFlight &&
          result.transcriptStatus === "fetching" &&
          result.transcriptRunId
        ) {
          const reconnect = await getRunReconnectionData(
            episode.id,
            "transcript",
          );
          if (!cancelled) {
            if (reconnect.ok) {
              ...
              if (
                !cancelled &&
                fresh.ok &&
                fresh.transcriptStatus !== null &&
                fresh.transcriptStatus !== "fetching"
              ) {
  ...
  const handleFetchTranscript = async () => {
    ...
    try {
      ...
      setTranscriptMsg(
        err instanceof Error ? err.message : "Failed to start transcript fetch",
      );
    }
  };
  ...
  const handleFetchAndSummarize = () => {
    if (isCombinedRef.current || transcriptInProgress || summaryInProgress)
      return;
          ...
          <TooltipTrigger asChild>
            <span className="inline-block">
              <Button
                ...
                disabled={
                  !transcriptAvailable || summaryInProgress || isCombinedAction
                }
