...
vi.mock("@/app/actions/library", () => ({
  getLibraryEntryByEpisodeId: (...args: unknown[]) => mockGetLibraryEntry(...args),
...
describe("BookmarkButton", () => {
  beforeEach(() => {
    ...
    mockState.currentEpisode = { id: "ep-123", title: "Test Episode", podcastTitle: "Test Pod", audioUrl: "http://example.com/a.mp3" }
    mockGetLibraryEntry.mockResolvedValue({ libraryEntryId: 42, episodeId: 10 })
  ...
  it("calls addBookmark with floored timestamp on click", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Bookmark current position" }))
