...
describe("getQueue", () => {
  ...
  it(
    ...
    testDbError(mockFindMany, async () => (await importAction()).getQueue()),
  );
});
...
describe("setQueue", () => {
  ...
  it("skips db.batch and returns error when ensureUserExists rejects", async () => {
    mockEnsureUserExists.mockRejectedValueOnce(
      new Error("user provisioning failed"),
    );
