...
describe("POST /api/admin/test-prompt", () => {
  beforeEach(() => {
    ...
    mockGetActiveAiConfig.mockResolvedValue({
      provider: "openrouter",
      model: "gpt-4",
    });
  ...
  it("returns 403 for non-admin", async () => {
    ...
    const res = await POST(
      makeRequest({ prompt: "test {{transcript}}", episodeId: 1 }),
    );
  ...
  it("returns 422 when prompt is missing {{transcript}}", async () => {
    const res = await POST(
      makeRequest({ prompt: "no placeholder here", episodeId: 1 }),
    );
  ...
  it("returns 422 when episode has no transcript", async () => {
    ...
    const res = await POST(
      makeRequest({ prompt: "{{transcript}}", episodeId: 1 }),
    );
    expect(res.status).toBe(422);
  });
  ...
  it("returns 422 when episode not found", async () => {
    ...
    const res = await POST(
      makeRequest({ prompt: "{{transcript}}", episodeId: 999 }),
    );
    expect(res.status).toBe(422);
  });
  ...
  it("returns streaming response with correct Content-Type for valid request", async () => {
    const res = await POST(
      makeRequest({ prompt: "Analyze {{transcript}}", episodeId: 1 }),
    );
