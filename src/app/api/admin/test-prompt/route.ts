...
export async function POST(request: Request) {
  ...
  if (prompt.length > 10000) {
    return new Response("prompt must be 10,000 characters or fewer", { status: 422 })
  }
  ...
  if (typeof episodeId !== "number" || !Number.isInteger(episodeId) || episodeId <= 0) {
    return new Response("episodeId must be a positive integer", { status: 422 })
  }
  ...
  const interpolated = interpolatePrompt(prompt, {
    title: episode.title,
    podcastName: (episode.podcast as { title?: string } | undefined)?.title ?? "Unknown Podcast",
