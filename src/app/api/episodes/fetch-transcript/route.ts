...
export async function POST(request: NextRequest) {
  try {
    ...
    const { episodeId, podcastIndexId: rawPodcastIndexId } = body as {
      episodeId?: unknown;
      podcastIndexId?: unknown;
    };
    ...
    if (episodeId !== undefined) {
      ...
      if (
        !Number.isInteger(numericPodcastIndexId) ||
        numericPodcastIndexId <= 0
      ) {
        return NextResponse.json(
          {
            error:
              "Episode has a non-numeric PodcastIndex ID and cannot be fetched via this endpoint",
          },
          { status: 400 },
        );
      }
      ...
      if (
        typeof rawPodcastIndexId === "string" &&
        rawPodcastIndexId.startsWith("rss-")
      ) {
        return NextResponse.json(
          {
            error:
      ...
      if (
        !Number.isInteger(parsedPodcastIndexId) ||
        parsedPodcastIndexId <= 0
      ) {
        return NextResponse.json(
          { error: "A valid positive podcastIndexId is required" },
          { status: 400 },
      ...
      if (existingEpisode) {
        ...
        try {
          ...
          console.error("PodcastIndex getEpisodeById failed:", {
            podcastIndexId: numericPodcastIndexId,
            error: err instanceof Error ? err.message : String(err),
          });
          return NextResponse.json(
            { error: "Failed to look up episode in PodcastIndex" },
            { status: 502 },
          );
        }
        if (!piEpisode) {
          return NextResponse.json(
            { error: "Episode not found in PodcastIndex" },
            { status: 404 },
          );
        }
        ...
        try {
          ...
          const categoryValues = piPodcast.categories
            ? Object.values(piPodcast.categories)
            : [];
          podcastDbId = await upsertPodcast(
            {
              ...
              categories:
                categoryValues.length > 0 ? categoryValues : undefined,
          ...
          console.error("Failed to fetch/upsert podcast:", {
            feedId: piEpisode.feedId,
            error: err instanceof Error ? err.message : String(err),
          });
          return NextResponse.json(
            { error: "Failed to fetch podcast data from PodcastIndex" },
            { status: 502 },
          );
        }

        // Insert episode stub — leave transcriptStatus as null until the task is
        // actually queued, so a failed trigger doesn't leave the row stuck in "fetching".
        ...
        try {
          ...
          console.error("Failed to insert episode stub:", {
            podcastIndexId: podcastIndexIdStr,
            error: err instanceof Error ? err.message : String(err),
          });
          return NextResponse.json(
    ...
    // Set transcriptStatus and transcriptRunId together AFTER the task is queued.
    // This prevents rows getting stuck in "fetching" with no runId if trigger fails.
    try {
      await db
        .update(episodes)
        .set({
      ...
      console.error("Failed to update episode after trigger:", {
