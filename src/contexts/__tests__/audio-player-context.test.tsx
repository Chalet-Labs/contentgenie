...
const mockLoadPrefs = vi
  .fn()
  .mockReturnValue({ volume: 0.8, playbackSpeed: 1.5 });
...
function TestConsumer() {
  ...
  return (
    <div>
      ...
      <span data-testid="episodeTitle">
        {state.currentEpisode?.title ?? ""}
      </span>
      <span data-testid="queueLength">{state.queue.length}</span>
      <span data-testid="queueIds">
        {state.queue.map((ep) => ep.id).join(",")}
      </span>
...
describe("Queue state management", () => {
    ...
    function CurrentEpisodeQueueConsumer() {
      ...
      return (
        <div>
          ...
          <button onClick={() => api.playEpisode(mockEpisode)}>
            Play Episode
          </button>
          <button onClick={() => api.addToQueue(mockEpisode)}>
            Add Playing To Queue
          </button>
          <button onClick={() => api.addToQueue(queueEpisode1)}>Add Q1</button>
        </div>
      );
  ...
  it("queue persists to localStorage on mutation", async () => {
    ...
    const lastCall =
      mockSaveQueue.mock.calls[mockSaveQueue.mock.calls.length - 1];
...
describe("hooks throw outside provider", () => {
  ...
  it("useIsEpisodePlaying throws without provider", () => {
    function BadConsumer() {
      useIsEpisodePlaying("any");
      return null;
    }
    expect(() => render(<BadConsumer />)).toThrow(
...
function ChapterConsumer() {
  ...
  return (
    <div>
      <span data-testid="chapters">
        {state.chapters ? JSON.stringify(state.chapters) : "null"}
      </span>
...
describe("Chapter state management", () => {
  ...
  it("dispatches SET_CHAPTERS on successful chapter fetch", async () => {
    ...
    expect(screen.getByTestId("chaptersLoading")).toHaveTextContent("false");
    expect(screen.getByTestId("chapters")).toHaveTextContent(
...
const mockGetPlayerSession = vi
  .fn()
  .mockResolvedValue({ success: true, data: null });
const mockSavePlayerSessionAction = vi
  .fn()
  .mockResolvedValue({ success: true });
const mockClearPlayerSessionAction = vi
  .fn()
  .mockResolvedValue({ success: true });
vi.mock("@/app/actions/player-session", () => ({
  getPlayerSession: (...args: unknown[]) => mockGetPlayerSession(...args),
  savePlayerSession: (...args: unknown[]) =>
    mockSavePlayerSessionAction(...args),
  clearPlayerSession: (...args: unknown[]) =>
    mockClearPlayerSessionAction(...args),
}));
  ...
  it("calls recordListenEvent with completed:true on ended event", async () => {
    ...
    expect(mockRecordListenEvent).toHaveBeenCalledWith(
      expect.objectContaining({
        completed: true,
        podcastIndexEpisodeId: mockEpisode.id,
      }),
    );
  });
...
describe("Cross-device sync: hydration and reconcile", () => {
  ...
  it("fires a toast when the migration upload fails", async () => {
    ...
    mockLoadQueue.mockReturnValue(localQueue);
    mockGetQueue.mockResolvedValue({ success: true, data: [] });
    mockSetQueueAction.mockResolvedValueOnce({
      success: false,
      error: "DB error",
    });
