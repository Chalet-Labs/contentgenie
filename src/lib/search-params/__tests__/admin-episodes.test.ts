...
describe("loadAdminEpisodeSearchParams", () => {
  ...
  it("parses comma-separated transcript statuses", () => {
    const result = loadAdminEpisodeSearchParams({ transcriptStatus: "available,failed" })
    expect(result.transcriptStatus).toEqual(["available", "failed"])
  })

  it("parses single transcript status as array", () => {
    const result = loadAdminEpisodeSearchParams({ transcriptStatus: "available" })
    expect(result.transcriptStatus).toEqual(["available"])
  })

  it("parses comma-separated summary statuses", () => {
    const result = loadAdminEpisodeSearchParams({ summaryStatus: "queued,completed" })
    expect(result.summaryStatus).toEqual(["queued", "completed"])
  })

  it("parses podcastId as integer", () => {
    expect(loadAdminEpisodeSearchParams({ podcastId: "42" }).podcastId).toBe(42)
  })

  it("returns null for non-numeric podcastId", () => {
    expect(loadAdminEpisodeSearchParams({ podcastId: "abc" }).podcastId).toBeNull()
  })

  it("parses ISO date strings", () => {
    const result = loadAdminEpisodeSearchParams({ dateFrom: "2026-01-15", dateTo: "2026-03-01" })
