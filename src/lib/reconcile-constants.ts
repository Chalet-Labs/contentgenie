 ...
 *
 * See ADR-048 for the rationale behind each value.
 */
 ...
 * window the resolver's conservative ingestion path missed. ADR-048 §1.
 */
export const RECONCILE_DBSCAN_EPS = 0.1;

/**
 * Minimum DBSCAN cluster size. Matches the issue spec — singleton "clusters"
 * are dropped before any LLM call. ADR-048 §1.
 */
 ...
 * whitelist + `ongoing=false` predicate). ADR-042 §"Decay" + ADR-048 §5.
 */
 ...
 * even on a budget-exhausted run. ADR-048 §6.
 */
 ...
 * issue spec lists; `other` is omitted (deviation documented in ADR-048 §5).
 */
