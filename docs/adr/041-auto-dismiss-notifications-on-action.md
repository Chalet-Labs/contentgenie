...
- **`addToQueue` dispatch carries `{ episodeDbIds: [] }` (intentional empty array).** The audio-player client only knows `PodcastIndexEpisodeId` (text) for the episode being queued; the integer `episodes.id` lives behind a server lookup. Asking the client to make an extra round-trip just to enrich the event payload defeats the optimistic-UX goal. Solution: when `addToQueue` dispatches an empty `episodeDbIds`, the `NotificationPageList` subscriber falls back to `router.refresh()` AND a `getNotifications` re-fetch (replacing local `items`). The other two dispatch sites (`ListenedButton` after success, `onEnded` after `recordListenEvent` resolves) DO carry the resolved id because the server action returns it. To support that, `recordListenEvent`'s success result is extended with `data.episodeDbId`.
- **Why the second dispatch after the debounced `setQueue`?** Without it, the bell would self-correct only on the 60s poll. The audio-player provider observes the debounced `setQueue` resolution path and fires a second `NOTIFICATIONS_CHANGED_EVENT` (this time with the server-resolved `episodeDbIds`) so subscribers reconcile against truth.
...
- Four dispatch sites in client code:
  1. `ListenedButton` (after success) — `detail: { episodeDbIds: [<resolved id from action result>] }`.
  2. `audio-player-context.addToQueue` (optimistic, immediate) — `detail: { episodeDbIds: [] }` because the client doesn't know the integer id; subscribers fall back to a server re-fetch.
  3. `audio-player-context` debounced `setQueue` resolution — `detail: { episodeDbIds: [...] }` populated from a new `data.dismissedEpisodeDbIds: number[]` field on `setQueue`'s success response (added for this feature).
  4. `audio-player-context.onEnded` after `recordListenEvent` resolves — `detail: { episodeDbIds: [<id>] }`.
- Two subscribers:
  - `NotificationBell` — payload-agnostic; calls `fetchUnreadCount`.
  - `NotificationPageList` — when `detail.episodeDbIds.length > 0`, filters local `items` state directly (`setItems(prev => prev.filter(...))`); when empty, falls back to `router.refresh()` + a `getNotifications` re-fetch that replaces `items`. Always also calls `router.refresh()` so the SSR snapshot is fresh on the next navigation.
...
- The dispatch from `audio-player-context.onEnded` fires the new `NOTIFICATIONS_CHANGED_EVENT` even when no `summary_completed`/`new_episode` row exists for the played episode. The dispatch always carries a populated `episodeDbIds: [<id>]` (dispatch site #4 above), so `NotificationPageList` runs the filter path — a no-op when no local items match the id. The empty-payload re-fetch fallback documented under Decision Notes only fires from dispatch site #2 (`addToQueue` optimistic), never from `onEnded`. Net effect: no-op for the bell (count unchanged) and for `/notifications` (filter no-op). Acceptable cost for the simpler dispatch contract.
