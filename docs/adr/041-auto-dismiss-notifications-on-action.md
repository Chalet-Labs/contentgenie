...
- **Why a separate non-`"use server"` module?** Every exported `async function` from a `"use server"` file becomes a wire-callable RSC server-action endpoint. The dismiss helper takes `userId` as a plain parameter and contains no `auth()` call (intentionally — its callers have already authenticated). If it lived in `notifications.ts`, a network attacker could invoke it with arbitrary user IDs and dismiss any user's notifications. Moving it to `src/app/actions/_internal/dismiss-notifications.ts` (no `"use server"` directive) makes it a regular server-side import only — the function is unreachable from the wire.
