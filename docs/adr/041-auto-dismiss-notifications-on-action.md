...
- `recordListenEvent` return shape changes: `ActionResult` → `ActionResult<{ episodeDbId: number }>`. The integer id is already resolved earlier in the action body; threading it back to the response is a one-line addition. Existing call sites that destructure only `success` / `error` are unaffected.
...
- The dispatch from `audio-player-context.onEnded` fires the new `NOTIFICATIONS_CHANGED_EVENT` even when no `summary_completed`/`new_episode` row exists for the played episode — that's a no-op for the bell (count is unchanged) and for `/notifications` (filter is a no-op when no rows match the id). Acceptable cost for the simpler dispatch contract.
