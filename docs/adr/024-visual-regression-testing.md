...
- **Flaky tests from animation or async rendering:** Some stories may have loading states or CSS transitions that produce different screenshots on different runs. Mitigation: add `waitForLoadState('networkidle')` and `page.waitForTimeout()` in the test if needed; mark known-flaky stories as skipped until fixed.
