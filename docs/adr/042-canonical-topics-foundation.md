...
- **Event-type recency filter is narrower than the dormancy window.** The kNN candidate filter keeps event-type kinds (release, incident, regulation, announcement, deal, event, other) only when `last_seen > now() - interval '90 days'` or `ongoing=true`, while dormancy decay does not flip `status='dormant'` until 180 days. This creates a 91–180 day window in which a canonical is `status='active'` but excluded from kNN — a fresh mention of a 4-month-old event-type entity reliably creates a duplicate canonical, which reconciliation later merges (typical lag ≤24h). The same UX issue also applies to dormant rows past 180 days. Concept and work kinds are exempt at both windows: the kNN filter unconditionally includes them and the dormancy rule never decays them, so evergreen topics like "creatine" or "strength training" match correctly regardless of last-seen age. Recurring event-type entities (Bitcoin halving, WWDC) opt out of decay via `ongoing=true`. The narrower kNN window is an intentional recall budget — widening to 180d would roughly double the post-filter survivor set on event-heavy domains and pressure `ef_search`. Revisit when EPIC A produces ingestion-volume data.
