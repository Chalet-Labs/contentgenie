...
- Sub-runs cap progress visibility at the parent's `metadata`. A single task that uses `metadata.set/increment` at the per-episode granularity gives the dashboard the same live-progress UX without extra infra.
...
- **`metadata.append` size limit (~256 KB per run).** Backfilling 50 episodes per run × small per-episode log entries stays well under the cap. Larger batches (`batchSize` >> 50) risk truncation; the inline doc on the payload type warns callers.
