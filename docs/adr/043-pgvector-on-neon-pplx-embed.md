...
3. **Version-gate the resolver during the swap.** Cosine similarity is only meaningful within a single embedding model's output space — comparing a `pplx-embed-v1-0.6b` vector against a successor-model vector is mathematically noise. To prevent cross-space matches during migration, the resolver's kNN WHERE clause adds `embedding_model_version = $current_model_id` for the duration of the swap. The trade-off is structurally bounded: the resolver only sees same-space candidates, so it cannot produce false auto-matches across spaces; in exchange, in-flight ingests that target a canonical embedded under the old model after the writer has cut over will create a new canonical (over-creation), which the existing reconciliation merge path collapses. Over-creation is reversible — cross-space false matches are not. The predicate is essentially free at steady state (post-filter integer comparison on ≤200 survivors), so it can remain as defensive belt-and-suspenders after migration completes; thresholds may still need re-tuning via `entity-resolution-constants.ts` once the table is uniform.
