...
| Alternative                                            | Why not                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dedicated vector DB (Pinecone / Qdrant / Weaviate)** | Specialized and battle-tested at scale, but at <1M vectors pgvector on the existing Neon database is operationally equivalent for zero new infrastructure cost. Adds an operational service to monitor, secure, sync, and pay for. Reconsider when total retained canonicals / indexed vectors approach 1M (or earlier if archival/deletion does not keep total rows near active rows), or when index memory growth on Neon becomes the binding constraint. |
...
1. **Version-gate the resolver as a precondition.** Cosine similarity is only meaningful within a single embedding model's output space — comparing a `pplx-embed-v1-0.6b` vector against a successor-model vector is mathematically noise. Before any writer cutover, deploy the resolver's kNN WHERE clause with `embedding_model_version = $current_model_id` so candidate retrieval is structurally restricted to same-space canonicals. With the gate in place ahead of steps 2 and 3, the swap proceeds without a mixed-space window: queries embedded under the in-flight model only ever match canonicals embedded under the same model. The trade-off is structurally bounded: the resolver only sees same-space candidates, so it cannot produce false auto-matches across spaces; in exchange, in-flight ingests that target a canonical embedded under the old model after the writer has cut over will create a new canonical (over-creation), which the existing reconciliation merge path collapses. Over-creation is reversible — cross-space false matches are not. The predicate is essentially free at steady state (post-filter text equality comparison on ≤200 survivors), so it remains as defensive belt-and-suspenders after migration completes; thresholds may still need re-tuning via `entity-resolution-constants.ts` once the table is uniform.
2. **Cut over the writer.** Update `EMBEDDING_MODEL` in `src/lib/ai/embed.ts` to the new model id. Default `embedding_model_version` for new inserts changes to the new tag. Safe because step 1's gate is already live: new-model queries are filtered to new-model canonicals even while old-model canonicals still dominate the table.
3. **Filtered re-embed via Trigger.dev backfill.** Batch-iterate `WHERE embedding_model_version = 'perplexity/pplx-embed-v1-0.6b'` via `generateEmbeddings(texts[])` (the helper exposed in `src/lib/ai/embed.ts` already supports batch input). For each batch, regenerate `identity_embedding` and `context_embedding` from the source `label`, `aliases`, and `summary`; UPDATE rows in a single transaction and stamp the new `embedding_model_version`. Bounded throughput; can run for days without affecting the resolver because the gate from step 1 keeps every query inside its own embedding space.
...
- Spec: `.dev/pm/specs/2026-04-25-canonical-topics-foundation.md` (internal) — risks **R1**, **R6**, **R8**, **R9**, **R10**; planned threshold-tuning hook in `src/lib/entity-resolution-constants.ts` (to land in EPIC A); HNSW index DDL specifics.
