...
- **Take advisory lock before the unmerge preflight read.** Rejected — `previousWinnerId` is unknown until the read of `merged_into_id`, so the sorted-pair key cannot be computed yet. The preflight uses `SELECT … FOR UPDATE` for row-level coverage; the advisory lock follows once both IDs are known.
