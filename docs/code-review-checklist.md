...
- **`pre-pr-validation` integration:** this checklist is loaded in Phase 2.0 of [`.claude/skills/pre-pr-validation/SKILL.md`](../.claude/skills/pre-pr-validation/SKILL.md) and threaded into Codex review, `/pr-review-toolkit:review-pr`, and `/simplify` as a supplementary rubric (each layer keeps its existing scope). Reviewers are asked to cite section numbers as `[checklist §N]`. Graceful skip if the file is missing. Wired up in [#367](https://github.com/Chalet-Labs/contentgenie/issues/367).
